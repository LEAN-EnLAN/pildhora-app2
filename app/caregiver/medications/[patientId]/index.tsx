...
export default function CaregiverMedicationsIndex() {
  ...
  useEffect(() => {
    if (pid) dispatch(fetchMedications(pid));
  }, [pid]);
  ...
  const renderItem = ({ item }: { item: Medication }) => (
    <Card style={styles.card}>
      ...
      <Button variant="secondary" onPress={() => router.push(`/caregiver/medications/${pid}/${item.id}`)}>Ver Detalles</Button>
    </Card>
  );
...
const styles = StyleSheet.create({
  ...
  medDetail: { fontSize: 14, color: '#6B7280' },
});
